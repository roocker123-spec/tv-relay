...
// ---------- track last entry side per product ----------
/**
 * Map of product_symbol (e.g. "BTCUSD") -> 'buy' | 'sell'
 * Used to auto-correct BATCH TP sides to the opposite (reduce_only).
 */
...
/**
 * placeBatch with auto-correct:
 * - If we know last entry side for this product, force each order to:
 *     side = OPPOSITE(lastSide)
 *     reduce_only = true (unless user already set true)
 */
async function placeBatch(m){
  ...
  try {
    ...
    if (last && Array.isArray(body.orders)) {
      const opp = oppositeSide(last);
      body.orders = body.orders.map(o => {
        const oo = { ...o };
        if (!oo.side || String(oo.side).toLowerCase() === last) {
          oo.side = opp; // force opposite side
        }
...
/**
 * listOpenOrdersAllPages – robust multipage fetch
 */
...
/**
 * listPositionsArray – robust, quiet on schema change
 * 1) try /v2/positions
 * 2) try /v2/positions/margined
 * 3) return []
 */
 ...
 * Poll errors are logged but ignored.
 */
...
app.post('/tv', async (req, res) => {
  try {
    ...
    // 2) Batch limits passthrough (TPs from Pine) — with auto-correct
    if (msg.orders) {
